...
function HandsontableColumnSorting() {
  ...
  this.afterInit = function () {
    ...
    if (this.getSettings().columnSorting) {
      ...
      this.rootElement.on('click.handsontable', '.columnSorting', function (e) {
        var $target = $(e.target) , $parent = $(e.target.parentNode);
        if ($target.is('.columnSorting') || $parent.is('.columnSorting')) {
  ...
  this.sort = function () {
    ...
    this.sortIndex.sort(function (a, b) {
      ...
      if (instance.getSettings().columns && instance.getSettings().columns[instance.sortColumn].type === 'custom' && instance.getSettings().columns[instance.sortColumn].sortFunction) {
          var data = instance.getSettings().columns[instance.sortColumn].sortFunction(a[1],b[1]);
          a[1] = data.a;
          b[1] = data.b;
      }
...
Walkontable.prototype.canScrollVertical = function (delta) {
    return this.wtScroll.canScrollVertical(delta);
};

Walkontable.prototype.canScrollHorizontal = function (delta) {
    return this.wtScroll.canScrollHorizontal(delta);
};
...
WalkontableScroll.prototype.canScrollVertical = function (delta) {
...
WalkontableScroll.prototype.canScrollHorizontal = function (delta) {
    if (!this.instance.drawn) {
  ...
  $(this.instance.wtTable.TABLE).on('mousewheel', function (event, delta, deltaX, deltaY) {
    ...
    //wheelTimeout = setTimeout(function () { //timeout is needed because with fast-wheel scrolling mousewheel event comes dozen times per second
      if (deltaY) {
        ...
        if (that.instance.canScrollVertical(-Math.ceil(deltaY))) {
              event.preventDefault();
        }
        that.instance.scrollVertical(-Math.ceil(deltaY)).draw();
      }
      else if (deltaX) {
          if (that.instance.canScrollHorizontal(Math.ceil(Math.abs(deltaX)))) {
              event.preventDefault();
          }
          if (deltaX < 0) {
              that.instance.scrollHorizontal(-1*Math.ceil(Math.abs(deltaX))).draw();
          } else {
